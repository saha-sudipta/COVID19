...
city_zips <- read_csv("city_zips.csv")
cases_zip_data <- read_csv("All_cities_zip.csv") %>% select(1:5) 
```
...
vars <- c(# Essential industry workers
  "C24010_001", "C24010_012", "C24010_016", "C24010_020", "C24010_021", "C24010_024",
          ...
          #Total, #Entered after 2010, foreign born not a citizen
          "B05005_001", "B05005_006",
          #Total, #Not citizen
...
covar_zip_data <- covar_zip_data_raw %>%
  ...
  mutate(mhi=B19013_001,
         ...
         pct_new_imm=B05005_006/B05005_001,
         pct_non_cit=B05001_006/B05001_001,
         
         GEOID=as.character(GEOID)) %>% 
  select(GEOID, mhi, pct_pov, pct_col, total_pop, pct_unins, pct_essworkers, pct_overcrowded, pct_transit,
         pct_old, pct_new_imm, pct_non_cit)
```
...
geog_file <- read_csv("zcta_tract_rel_10.csv") %>% select(ZCTA5, GEOID) %>% mutate(GEOID=as.character(GEOID))
health_data_raw <- do.call(rbind,
          lapply(paste0("Health_data/", (list.files(path = "Health_data/"))), read.csv)) %>%
...
all_data <- covar_zip_data %>% 
  ...
  left_join(cases_zip_data, by="zip")
```
